...
# Google Drive 파일 ID
file_id = '1If6kZq3c33h1QgQk3wanzDJTqw7Q3dyD'
    ...
    for label, prob in zip(labels, probs):
        st.markdown(f"""
            <div style="background-color: #f0f0f0; border-radius: 5px; padding: 5px; margin: 5px 0;">
                ...
                <div style="background-color: #8258FA; border-radius: 5px; width: 100%; padding: 2px;">
...
def display_right_content(labels):
    ...
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.video("https://www.youtube.com/watch?v=EObXvNN6ms8&list=PLMpWEUOlusXbW4PDOAZiiLlRHVY_jd2JN", start_time=0)
            st.caption(f"유튜브: {label}")

    # 3rd Row - Text based on labels
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.write(f"{label} 으하하하 하히히히히히 .")
